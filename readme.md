...
* **[Priority 1: SIM-004-v2 — Electron CCR & GP Equilibrium](./issues/foundational/test-spec-electron-ccr-gp-equilibrium.yaml)**
  ...
  * **Falsification Threshold:** $\Omega_{\text{rotor, numerical}}$ must reproduce $\alpha^{-1} \approx 137.036$ dynamically from the phase gradient. *(See [Methodological Note on Numerology](./issues/foundational/NUMEROLOGY_VS_DERIVATION.md))*.
